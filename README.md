# 📚 Estudos de Redes de Computadores

Repositório dedicado ao armazenamento de conceitos, resumos teóricos e anotações aprendidos durante o curso de introdução a redes.

---

## 🧭 Sumário de Conteúdos

### Parte 1: Introdução e Infraestrutura Básica
- [ ] 🚀 [Introdução ao curso](./00-introducao/README.md)
- [ ] 📡 [Módulo 01: Comunicação em um Mundo Conectado](./01-comunicacao-mundo-conectado/README.md)
- [ ] 🖥️ [Módulo 02: Componentes, tipos e conexões de rede](./02-componentes-tipos-conexoes/README.md)
- [ ] 📶 [Módulo 03: Redes sem fio e móveis](./03-redes-sem-fio-e-moveis/README.md)
- [ ] 🏠 [Módulo 04: Construindo uma Rede Doméstica](./04-construindo-rede-domestica/README.md)
- [ ] 🧪 **Ponto de Verificação:** Construir uma pequena rede

### Parte 2: Transmissão e Camada de Acesso
- [ ] 💬 [Módulo 05: Princípios de comunicação](./05-principios-comunicacao/README.md)
- [ ] 🔌 [Módulo 06: Mídia de rede](./06-midia-de-rede/README.md)
- [ ] 🔑 [Módulo 07: A Camada de Acesso](./07-camada-de-acesso/README.md)
- [ ] 🧪 **Ponto de Verificação:** Acesso à rede

### Parte 3: Endereçamento IP e Protocolos
- [ ] 🌐 [Módulo 08: Protocolo de Internet (Internet Protocol - IP)](./08-protocolo-ip/README.md)
- [ ] 🔢 [Módulo 09: IPv4 e segmentação de rede](./09-ipv4-segmentacao/README.md)
- [ ] ⏩ [Módulo 10: Formatos e regras de endereçamento IPv6](./10-enderecamento-ipv6/README.md)
- [ ] 🔄 [Módulo 11: Endereçamento dinâmico com DHCP](./11-enderecamento-dinamico-dhcp/README.md)
- [ ] 🧪 **Ponto de Verificação:** O protocolo da Internet

### Parte 4: Interconexão e Roteamento
- [ ] 🚪 [Módulo 12: Gateways para outras redes](./12-gateways-outras-redes/README.md)
- [ ] 🔍 [Módulo 13: O Processo de ARP](./13-processo-arp/README.md)
- [ ] 🔀 [Módulo 14: Roteamento entre redes](./14-roteamento-entre-redes/README.md)

---

## 📁 Estrutura de Pastas

```text
.
├── 00-introducao/
├── 01-comunicacao-mundo-conectado/
├── 02-componentes-tipos-conexoes/
├── 03-redes-sem-fio-e-moveis/
├── 04-construindo-rede-domestica/
├── 05-principios-comunicacao/
├── 06-midia-de-rede/
├── 07-camada-de-acesso/
├── 08-protocolo-ip/
├── 09-ipv4-segmentacao/
├── 10-enderecamento-ipv6/
├── 11-enderecamento-dinamico-dhcp/
├── 12-gateways-outras-redes/
├── 13-processo-arp/
├── 14-roteamento-entre-redes/
└── README.md
